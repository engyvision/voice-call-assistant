/*
  # Create Call Records Table with RLS
  ...
  2. Security
    ...
    - Add public access policy (temporary for development)
    - Add authenticated user policies for future use
    - Add automatic trigger for completed_at timestamp

  3. Performance
    - Add indexes for common queries (created_at, status, user_id, phone_number)
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Allow public access to call records'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'call_records' 
    AND policyname = 'Users can insert own call records'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'call_records' 
    AND policyname = 'Users can update own call records'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'call_records' 
    AND policyname = 'Users can delete own call records'
  ) THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM call_records LIMIT 1) THEN
    ...
    (
      ...
      'Assistant: Hello, I''d like to book a dental cleaning appointment for my client...' || E'\n' ||
      'Receptionist: Certainly! What day works best for you?' || E'\n' ||
    ...
    (
      ...
      'Assistant: I''d like to make a reservation for 4 people on Friday evening...' || E'\n' ||
    ...
    (
      ...
      'Assistant: Could you please tell me about your weekend hours?' || E'\n' ||
