/*
  # Complete Call Records Schema Setup
  ...
  2. Security
    ...
    - Add public access policy for development/testing
...
DO $$
DECLARE
  sample_id_1 uuid := gen_random_uuid();
  sample_id_2 uuid := gen_random_uuid();
  sample_id_3 uuid := gen_random_uuid();
BEGIN
  -- Insert sample records only if they don't exist
  ...
  WHERE NOT EXISTS (
    ...
    WHERE recipient_name = 'Dr. Smith Medical Office' 
    AND phone_number = '+1-555-0123'
  );
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM call_records 
    WHERE recipient_name = 'Pizza Palace Restaurant' 
    AND phone_number = '+1-555-0456'
  );
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM call_records 
    WHERE recipient_name = 'City Library' 
    AND phone_number = '+1-555-0789'
  );
END $$;