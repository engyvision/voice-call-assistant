/*
  # Create call_records table with complete schema
...
-- Insert some sample data for testing (use ON CONFLICT to avoid duplicates)
...
(
  'sample-1-' || extract(epoch from now())::text::uuid,
...
(
  'sample-2-' || extract(epoch from now())::text::uuid,
...
(
  'sample-3-' || extract(epoch from now())::text::uuid,
  'City Library',
  '+1-555-0789',
  'Get information',
