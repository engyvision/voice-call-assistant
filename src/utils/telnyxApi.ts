...
export async function initiateCall(request: CallRequest): Promise<ApiResponse<string>> {
  try {
    ...
    // Then initiate Telnyx call via edge function
    const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/telnyx-initiate`, {
      method: 'POST',
      headers: {
...
export function simulateCallProgress(callId: string): void {
  ...
  console.log('Real call initiated, progress will be tracked via Telnyx webhooks');
}